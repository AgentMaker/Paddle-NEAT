...
class Node:
    def __init__(
        ...
        response: float
        bias: float
        activation: paddle function from .activations
        aggregation: paddle function from .aggregations
        name: str
    ...
    def activate(self, xs, shape):
        """
        xs: list of paddle tensors
        """
        if not xs:
            return paddle.full(shape, self.bias)
...
def clamp_weights_(weights, weight_threshold=0.2, weight_max=3.0):
    ...
    for _ in range(weights.shape[0]):
        if weights[_].abs() < weight_threshold:
            weights[_] = 0
        if weights[_] > 0:
            weights[_] -= weight_threshold
        elif weights[_] < 0:
            weights[_] += weight_threshold
        if weights[_] > weight_max:
            weights[_] = weight_max
        elif weights[_] < -weight_max:
            weights[_] = -weight_max
