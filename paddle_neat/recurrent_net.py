...
#     idxs, weights = conns
#     if len(idxs) > 0:
#         idxs = paddle.LongTensor(idxs).t()
#         weights = paddle.FloatTensor(weights)
#         mat = paddle.sparse.FloatTensor(idxs, weights, shape)
#     else:
#         mat = paddle.sparse.FloatTensor(shape[0], shape[1])
#     return mat


def dense_from_coo(shape, conns, dtype="float64"):
    mat = paddle.zeros(shape, dtype=dtype)
    idxs, weights = conns
    if len(idxs) == 0:
        return mat
    rows, cols = np.array(idxs).transpose()
    mat[rows, cols] = paddle.to_tensor(
        [weights], dtype=dtype)
    return mat
...
class RecurrentNet():
    def __init__(self, n_inputs, n_hidden, n_outputs,
        ...
        if n_hidden > 0:
            self.hidden_responses = paddle.to_tensor(hidden_responses, dtype=dtype)
            self.hidden_biases = paddle.to_tensor(hidden_biases, dtype=dtype)

        self.output_responses = paddle.to_tensor(
            output_responses, dtype=dtype)
        self.output_biases = paddle.to_tensor(output_biases, dtype=dtype)

        self.reset(batch_size)

    def reset(self, batch_size=1):
        if self.n_hidden > 0:
            self.activs = paddle.zeros(
                [batch_size, self.n_hidden], dtype=self.dtype)
        else:
            self.activs = None
        self.outputs = paddle.zeros(
    ...
    def activate(self, inputs):
        ...
        '''
        with paddle.no_grad():
            inputs = paddle.to_tensor([inputs], dtype=self.dtype).squeeze()
